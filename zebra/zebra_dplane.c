...
const struct rtmsg *dplane_ctx_get_route_rtm(struct zebra_dplane_ctx *ctx)
{
	DPLANE_CTX_VALID(ctx);
...
void dplane_ctx_mac_set_addr(struct zebra_dplane_ctx *ctx, struct ethaddr *mac)
{
	DPLANE_CTX_VALID(ctx);
...
void dplane_ctx_neigh_set_mac(struct zebra_dplane_ctx *ctx, struct ethaddr *mac)
{
	DPLANE_CTX_VALID(ctx);
