...
DEFINE_MTYPE_STATIC(ZEBRA, DP_MPATH, "Zebra DPlane Multipath Array");
...
parse_nexthop_unicast(ns_id_t ns_id, unsigned rtm_flags, struct rtattr **tb,
		      ...
		      void *gate, afi_t afi, vrf_id_t vrf_id)
{
	...
	if (rtm_flags & RTNH_F_ONLINK)
		SET_FLAG(nh.flags, NEXTHOP_FLAG_ONLINK);

	if (rtm_flags & RTNH_F_LINKDOWN)
		...
		if (gate && rtm_family == AF_INET) {
			if (index)
				...
				nh = nexthop_from_ipv4(gate, prefsrc,
						       nh_vrf_id);
		} else if (gate && rtm_family == AF_INET6) {
			if (index)
...
void zebra_route_dplane_update(struct zebra_dplane_ctx *ctx)
{
	...
	struct rtmsg *rtm = dplane_ctx_get_route_rtm(ctx);
	// uint8_t rtm_family = dplane_ctx_get_route_rtm_family(ctx);
	...
	uint16_t nlmsg_type = dplane_ctx_get_route_nlmsg_type(ctx);
	...
	if (IS_ZEBRA_DEBUG_KERNEL) {
		...
		zlog_debug(
			...
			nl_msg_type_to_str(nlmsg_type), &p,
	...
	struct rtattr **tb = dplane_ctx_get_route_tb(ctx);
	...
	if (op == DPLANE_OP_ROUTE_INSTALL) {
		...
		if (!tb[RTA_MULTIPATH]) {
			...
			struct rtnexthop *rtnh =
				// (struct rtnexthop *)multipath_array->rtnh;
						      ...
						      ns_id_t ns_id, int startup)
{
	...
	dplane_ctx_set_route_tb(ctx, tb);
	dplane_ctx_set_route_rtm(ctx, rtm);
	...
	dplane_ctx_set_route_nlmsg_type(ctx, h->nlmsg_type);
...
static int netlink_ipneigh_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	dplane_ctx_neigh_set_link_layer_ipv4(ctx, link_layer_ipv4);
	dplane_ctx_set_op(ctx, op);
	dplane_ctx_set_ifindex(ctx, ndm->ndm_ifindex);
	dplane_ctx_set_ns_id(ctx, ns_id);
	dplane_ctx_neigh_set_ndm_family(ctx, ndm->ndm_family);
	dplane_ctx_neigh_set_ndm_state(ctx, ndm->ndm_state);
	dplane_ctx_neigh_set_l2_len(ctx, l2_len);
