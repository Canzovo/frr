...
static struct nexthop parse_nexthop_unicast(ns_id_t ns_id, unsigned rtm_flags, struct rtattr **tb,
		...
		if (gate && rtm_family == AF_INET) {
			if (index)
				nh = nexthop_from_ipv4_ifindex(
					gate, prefsrc, index, 0);
			else
				nh = nexthop_from_ipv4(gate, prefsrc, 0);
		} else if (gate && rtm_family == AF_INET6) {
			if (index)
				nh = nexthop_from_ipv6_ifindex(
					gate, index, 0);
			else
				nh = nexthop_from_ipv6(gate, 0);
		} else
			nh = nexthop_from_ifindex(index, 0);
