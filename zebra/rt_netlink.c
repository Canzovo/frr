...
static int netlink_macfdb_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if (tb[NDA_DST]) {
		...
		memcpy(&vtep_ip.s_addr, RTA_DATA(tb[NDA_DST]),
		       IPV4_MAX_BYTELEN);
		snprintf(dst_buf, sizeof(dst_buf), " dst %pI4",
			   &vtep_ip);
	} else
	...
	dplane_ctx_set_ns_id(ctx, ndm->ndm_ifindex);
	dplane_ctx_set_ifindex(ctx, ndm->ndm_ifindex);
    dplane_ctx_set_fdb_nlmsg_type(ctx, h->nlmsg_type);
	
	memcpy(&mac, RTA_DATA(tb[NDA_LLADDR]), ETH_ALEN);
	dplane_ctx_set_fdb_mac(ctx, mac);
	dplane_ctx_set_fdb_nhg_id(ctx, nhg_id);
	dplane_ctx_set_fdb_ndm_state(ctx, ndm->ndm_state);
	dplane_ctx_set_fdb_ndm_flags(ctx, ndm->ndm_flags);
	dplane_ctx_set_fdb_dst_present(ctx, dst_present);
	dplane_ctx_set_fdb_vtep_ip(ctx, vtep_ip);
	dplane_ctx_set_fdb_vid(ctx, vid);
	dplane_ctx_set_fdb_nhg_id(ctx, nhg_id);
	...
	dplane_ctx_set_fdb_dp_static(ctx, dp_static);
	dplane_ctx_set_fdb_local_inactive(ctx, local_inactive);

	if (tb[NDA_SRC_VNI])
		vni = *(vni_t *)RTA_DATA(tb[NDA_SRC_VNI]);

	dplane_ctx_set_fdb_vni(ctx, vni);
	...
	dplane_ctx_set_fdb_is_sticky(ctx, sticky);
 ...
 */
void netlink_handle_5549(uint32_t ndm_family, uint32_t ndm_state,
...
static int netlink_ipneigh_change(struct nlmsghdr *h, int len, ns_id_t ns_id)
{
	...
	if(h->nlmsg_type != RTM_NEWNEIGH &&
	   h->nlmsg_type != RTM_DELNEIGH && h->nlmsg_type != RTM_GETNEIGH) {
		return 0;
	}
	...
	enum dplane_op_e op;
	if (h->nlmsg_type == RTM_NEWNEIGH) {
		op = DPLANE_OP_NEIGH_IP_INSTALL;
		cmd = ZEBRA_NEIGH_ADDED;
	} else if (h->nlmsg_type == RTM_DELNEIGH) {
		op = DPLANE_OP_NEIGH_IP_DELETE;
	...
	if (l2_len) {
		dplane_ctx_set_neigh_desc(ctx, RTA_DATA(tb[NDA_LLADDR]));
	}

	dplane_ctx_set_op(ctx, op);
	dplane_ctx_set_neigh_nlmsg_type(ctx, h->nlmsg_type);
	dplane_ctx_set_ifindex(ctx, ndm->ndm_ifindex);
	dplane_ctx_set_ns_id(ctx, ns_id);
	dplane_ctx_set_neigh_ndm_family(ctx, ndm->ndm_family);
	dplane_ctx_set_neigh_ndm_state(ctx, ndm->ndm_state);
	dplane_ctx_set_neigh_l2_len(ctx, l2_len);

	memset(&ip, 0, sizeof(ip));
	ip.ipa_type = (ndm->ndm_family == AF_INET) ? IPADDR_V4 : IPADDR_V6;
	memcpy(&ip.ip.addr, RTA_DATA(tb[NDA_DST]), RTA_PAYLOAD(tb[NDA_DST]));
	dplane_ctx_set_neigh_ipaddr(ctx, ip);
	dplane_ctx_set_neigh_cmd(ctx, cmd);
	...
	if (h->nlmsg_type == RTM_NEWNEIGH) {
		...
		dplane_ctx_set_neigh_mac(ctx, mac);
		dplane_ctx_set_neigh_mac_present(ctx, mac_present);
		...
		dplane_ctx_set_neigh_ext_flags(ctx, ext_flags);
		dplane_ctx_set_neigh_is_ext(ctx, is_ext);
		dplane_ctx_set_neigh_is_router(ctx, is_router);
		dplane_ctx_set_neigh_dp_static(ctx, dp_static);
		...
		if (ndm->ndm_state & NUD_VALID) {
			...
			dplane_ctx_set_neigh_local_inactive(ctx, local_inactive);
		}
	}
	dplane_provider_enqueue_to_zebra(ctx);
	return 0;
